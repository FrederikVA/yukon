...
static Button buttons[] = {
    ...
    {{0, 0, BUTTON_WIDTH, BUTTON_HEIGHT}, "SAVE DECK", "SD_GUI", 0},
...
static SDL_Rect savedDeckRects[SAVE_LIST_MAX];
static char savedDeckNames[SAVE_LIST_MAX][100];
static int savedDeckCount = 0;
...
static void drawStatusMessage(SDL_Renderer *renderer, TTF_Font *font, int screenHeight) {
    SDL_Color white = {245, 245, 245, 255};
    ...
    drawText(renderer, font, "Message:", 30, screenHeight - 72, muted);

    if (len <= maxChars) {
        drawText(renderer, font, message, 150, screenHeight - 72, white);
        return;
    }
    ...
    snprintf(firstLine, sizeof(firstLine), "%.*s", split, message);
    snprintf(secondLine, sizeof(secondLine), "%.*s", maxChars, message + split + (message[split] == ' ' ? 1 : 0));
    drawText(renderer, font, firstLine, 150, screenHeight - 88, white);
...
static void addSortedName(char names[SAVE_LIST_MAX][100], int *count, const char *name) {
    int insertAt = *count;

    if (*count >= SAVE_LIST_MAX) {
        return;
    }

    while (insertAt > 0 && strcmp(names[insertAt - 1], name) > 0) {
        snprintf(names[insertAt], sizeof(names[insertAt]), "%s", names[insertAt - 1]);
        insertAt--;
    }

    snprintf(names[insertAt], sizeof(names[insertAt]), "%s", name);
    (*count)++;
}
...
static void refreshSavedDeckList(void) {
    ...
    while ((entry = readdir(dp)) != NULL) {
        int len = (int)strlen(entry->d_name);
        if (len > 4 && strcmp(entry->d_name + len - 4, ".txt") == 0) {
            addSortedName(savedDeckNames, &savedDeckCount, entry->d_name);
        }
    }
    closedir(dp);
}
...
static void buildUniqueGuiDeckSaveCommand(char *command, int commandSize) {
    ...
    for (int i = 0; i < 100; i++) {
        if (i == 0) {
            snprintf(filename, sizeof(filename), "gui_deck_%ld.txt", now);
        } else {
            snprintf(filename, sizeof(filename), "gui_deck_%ld_%d.txt", now, i);
        }

        snprintf(path, sizeof(path), "decks/%s", filename);
        FILE *f = fopen(path, "r");
        if (!f) {
            snprintf(command, commandSize, "SD %s", filename);
            return;
        }
        fclose(f);
    }

    snprintf(command, commandSize, "SD gui_deck_%ld_fallback.txt", now);
}
...
void handleButtonClick(int x, int y) {
    for (int i = 0; i < buttonCount; ++i) {
        ...
        if (x >= btn->rect.x && x <= btn->rect.x + btn->rect.w &&
            ...
            if (strcmp(btn->command, "SD_GUI") == 0) {
                char command[120];
                buildUniqueGuiDeckSaveCommand(command, sizeof(command));
...
void drawStartupSavedGames(SDL_Renderer *renderer, TTF_Font *font, int screenWidth, int screenHeight) {
    ...
    SDL_Rect panel = {panelX - 10, y - 12, 250, screenHeight - 110};
    SDL_SetRenderDrawColor(renderer, 45, 45, 45, 255);
    SDL_RenderFillRect(renderer, &panel);
    SDL_SetRenderDrawColor(renderer, 100, 100, 100, 255);
    SDL_RenderDrawRect(renderer, &panel);

    drawText(renderer, font, "SAVED DECKS", panelX, y, white);
    y += 42;

    refreshSavedDeckList();
    for (int i = 0; i < savedDeckCount; i++) {
        savedDeckRects[i] = (SDL_Rect){panelX, y, 210, 32};
        SDL_SetRenderDrawColor(renderer, 58, 72, 78, 255);
        SDL_RenderFillRect(renderer, &savedDeckRects[i]);
        SDL_SetRenderDrawColor(renderer, 120, 140, 145, 255);
        SDL_RenderDrawRect(renderer, &savedDeckRects[i]);
        drawSavedGameName(renderer, font, savedDeckNames[i], panelX + 8, y + 3);
        y += 38;
    }

    if (savedDeckCount == 0) {
        drawText(renderer, font, "No saved decks", panelX, y, muted);
        y += 38;
    }
...
int handleSavedGameClick(int x, int y) {
    ...
    for (int i = 0; i < savedDeckCount; i++) {
        if (SDL_PointInRect(&point, &savedDeckRects[i])) {
            snprintf(command, sizeof(command), "LD %s", savedDeckNames[i]);
            handleCommand(command);
            return 1;
        }
    }
...
static int handleSavedStateClick(int x, int y) {
    SDL_Point point = {x, y};
    char command[120];
...
int handlePlayControlClick(int x, int y) {
    ...
    return handleSavedStateClick(x, y);
}
