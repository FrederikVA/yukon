...
static int findFoundationForCard(Card *card) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (top->suit == card->suit &&
            getRankIndex(card->rank) == getRankIndex(top->rank) + 1) {
            return i;
        }
    }
            ...
            if (e.type == SDL_QUIT) {
                ...
                if (currentPhase == STARTUP) {
                    ...
                    if (e.button.clicks == 2 && hoveredCard) {
                        ...
                        if (curr == hoveredCard) {
                            int foundationIndex = findFoundationForCard(hoveredCard);
                            char src[10], cmd[20];
                            snprintf(src, sizeof(src), "C%d:%c%c", colIndex + 1, hoveredCard->rank, hoveredCard->suit);
                    
                            if (foundationIndex >= 0) {
                                snprintf(cmd, sizeof(cmd), "%s->F%d", src, foundationIndex + 1);
